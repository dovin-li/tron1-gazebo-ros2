 ...
 * @file Tron2HWSim.cpp
 *
 * @brief This file defines the Tron2HWSim class, which is responsible for simulating a legged robot.
 *
...
#include "pointfoot_gazebo/Tron2HWSim.h"

// static pf disabled
static int motor_number = 10;  // hardcoded
...
class pointfoot_gazebo::Tron2GazeboSystemPrivate
{
public:
  Tron2GazeboSystemPrivate() = default;
...
namespace pointfoot_gazebo
{
      ...
      sdf::ElementPtr sdf)
  {
    ...
    pf = limxsdk::Tron2Sim::getInstance();
    // pf call removed

    motor_number = // pf call removed

    // Write the number of motors in the robotCmdBuffer_
    // robotCmdBuffer disabled

    // Subscribe to robot command messages and write them to robotCmdBuffer_
    // pf call removed });

    this->dataPtr_ = std::make_unique<Tron2GazeboSystemPrivate>();
  ...
  void Tron2HWSim::registerJoints(
      const hardware_interface::HardwareInfo &hardware_info,
      gazebo::physics::ModelPtr parent_model)
  {
  ...
  void Tron2HWSim::registerSensors(
      const hardware_interface::HardwareInfo &hardware_info,
      gazebo::physics::ModelPtr parent_model)
  {
  ...
  CallbackReturn
  Tron2HWSim::on_init(const hardware_interface::HardwareInfo &system_info)
  {
  ...
  Tron2HWSim::export_state_interfaces()
  {
    return std::move(this->dataPtr_->state_interfaces_);
  }

  std::vector<hardware_interface::CommandInterface>
  Tron2HWSim::export_command_interfaces()
  {
    return std::move(this->dataPtr_->command_interfaces_);
  }

  CallbackReturn Tron2HWSim::on_activate(const rclcpp_lifecycle::State &previous_state)
  {
    return CallbackReturn::SUCCESS;
  }

  CallbackReturn Tron2HWSim::on_deactivate(const rclcpp_lifecycle::State &previous_state)
  {
    return CallbackReturn::SUCCESS;
  }

  hardware_interface::return_type
  Tron2HWSim::perform_command_mode_switch(
  ...
  hardware_interface::return_type Tron2HWSim::read(
  ...
  hardware_interface::return_type Tron2HWSim::write(
      const rclcpp::Time &time,
      const rclcpp::Duration &period)
  {
  ...
  int Tron2HWSim::parseJointIndex(const std::string &jointName)
  {
    int leg_index = 0;
    int joint_index = 0;
...
PLUGINLIB_EXPORT_CLASS(pointfoot_gazebo::Tron2HWSim, gazebo_ros2_control::GazeboSystemInterface)
